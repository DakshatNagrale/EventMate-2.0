...
const SUMMARY_CARD_CONFIG = [
  ...
  },
  {
    ...
    title: "Deactivated",
    icon: CircleOff,
    iconClass: "text-rose-600 bg-rose-100 dark:text-rose-300 dark:bg-rose-500/20",
  },
];
...
export default function AdminOrganizerManagement() {
  ...
  const filteredOrganizers = useMemo(() => {
    ...
    return organizers.filter((organizer) => {
      return (
        ...
        organizer.department?.toLowerCase().includes(term) ||
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const nextOrganizer = response.data?.organizer;
      if (nextOrganizer) {
        upsertOrganizer(nextOrganizer);
      } else {
  ...
  const toggleOrganizerStatus = async (organizer) => {
    ...
    try {
      ...
      if (response.data?.organizer) {
        upsertOrganizer(response.data.organizer);
      } else {
        await fetchOrganizers();
      }
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                Admin creates and manages all organizers, including account status and permissions.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <label className="relative">
                ...
                <input
                  ...
                  className="w-full sm:w-56 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 py-2.5 pl-9 pr-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-300"
                />
              </label>
              <button
                type="button"
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">
                    ...
                    <th className="pb-3 pr-3">Total Events</th>
                    <th className="pb-3 pr-3">Force Deactivate</th>
                      ...
                      <tr key={organizer._id}>
                        <td className="py-3 pr-3">
                          <div className="flex items-center gap-3">
                            <div className="h-8 w-8 rounded-full overflow-hidden border border-slate-200 dark:border-white/10 bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-200 text-xs font-semibold flex items-center justify-center">
                              {organizer.avatar ? (
                                <img src={organizer.avatar} alt={organizer.fullName} className="h-full w-full object-cover" />
                              ) : (
                        ...
                        <td className="py-3 pr-3 text-slate-700 dark:text-slate-200">{organizer.totalEvents || 0} Events</td>
                        <td className="py-3 pr-3">
                          <button
                            type="button"
                            disabled={isPending}
                            onClick={() => toggleOrganizerStatus(organizer)}
                            className={`inline-flex h-5 w-9 items-center rounded-full p-0.5 transition disabled:opacity-60 ${
                              !organizer.isActive ? "bg-rose-500" : "bg-slate-200 dark:bg-slate-600/70"
                            }`}
                            aria-label={!organizer.isActive ? "Force deactivated" : "Not force deactivated"}
                          >
                            <span
                              className={`h-4 w-4 rounded-full bg-white transition ${
                                !organizer.isActive ? "translate-x-4" : "translate-x-0"
                              }`}
                            />
                          </button>
                        </td>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {SUMMARY_CARD_CONFIG.map((item) => {
            ...
            const value =
              item.id === "total" ? metrics.total : item.id === "active" ? metrics.active : metrics.deactivated;

            return (
              <article key={item.id} className="eventmate-kpi rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-gray-900/70 p-4">
                <div className="flex items-center gap-3">
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-300 mt-1">
                  {formMode === "create"
                    ? "Add a new organizer account from admin."
                    : "Update organizer account information."}
                </p>
              </div>
              <button
                type="button"
