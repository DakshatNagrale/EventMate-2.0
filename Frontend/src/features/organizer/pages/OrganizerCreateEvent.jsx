...
export default function OrganizerCreateEvent() {
  ...
  const [newCoordinatorForm, setNewCoordinatorForm] = useState(initialCoordinatorForm);
  ...
  const handleNewCoordinatorChange = (event) => {
    const { name, value } = event.target;
    setNewCoordinatorForm((prev) => ({ ...prev, [name]: value }));
  };

  const createNewCoordinator = async () => {
    if (creatingCoordinator) return;

    const fullName = newCoordinatorForm.fullName.trim();
    const email = newCoordinatorForm.email.trim().toLowerCase();
    const password = String(newCoordinatorForm.password || "");
    const mobileNumber = newCoordinatorForm.mobileNumber.trim();
    const scope = newCoordinatorForm.scope.trim();

    if (!fullName || !email || !password) {
      setMessage({ type: "error", text: "Coordinator name, email, and password are required." });
      return;
    }
    ...
    try {
      ...
      const created = response.data?.coordinator;
      if (created?._id) {
        setCoordinators((prev) => {
          const exists = prev.some((item) => item._id === created._id);
          return exists ? prev : [...prev, created].sort((a, b) => a.fullName.localeCompare(b.fullName));
        });
      ...
      setNewCoordinatorForm(initialCoordinatorForm);
      setMessage({ type: "success", text: response.data?.message || "Coordinator created and added." });
    } catch (error) {
      setMessage({
        type: "error",
            ...
            <div className="space-y-4">
              ...
              <section className="eventmate-panel rounded-xl border border-slate-200 dark:border-white/10 p-4">
                ...
                <div className="mt-4 rounded-lg border border-slate-200 dark:border-white/10 p-3">
                  <h3 className="text-xs font-semibold text-slate-900 dark:text-white">Create New Coordinator</h3>
                  <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input
                      ...
                      className="rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-2 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-300"
                    />
                    <input
                      name="email"
                      type="email"
                    ...
                    <input
                      name="password"
                      type="password"
                      value={newCoordinatorForm.password}
                      onChange={handleNewCoordinatorChange}
                    ...
                    <input
                      name="mobileNumber"
                      value={newCoordinatorForm.mobileNumber}
                      onChange={handleNewCoordinatorChange}
                    ...
                    <input
                      name="scope"
                      value={newCoordinatorForm.scope}
                      onChange={handleNewCoordinatorChange}
                      placeholder="Scope (optional)"
                      className="sm:col-span-2 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-2 text-xs text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-300"
                    />
                  </div>
                  <button
                    type="button"
                    onClick={createNewCoordinator}
                    disabled={creatingCoordinator}
                    className="mt-3 inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white hover:bg-indigo-700 disabled:opacity-70"
                  >
                    {creatingCoordinator ? <Loader2 size={12} className="animate-spin" /> : <Plus size={12} />}
                    {creatingCoordinator ? "Creating..." : "Create Coordinator"}
                  </button>
                </div>
              </section>
            </div>
                ...
                {form.participationMode !== "INDIVIDUAL" && (
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                      Includes team leader. Example: 4 means 1 leader + 3 members.
                    </p>
                  </div>
                )}
              </section>
