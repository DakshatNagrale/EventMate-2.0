...
export default function StudentEventDetails({ mode = "details" }) {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      ...
      setMessage(null);
      try {
        const [detailsResponse, myRegistrationsResponse] = await Promise.allSettled([
          api({
        ...
        const responseEvent = detailsResponse.value.data?.event;
        ...
        const registeredIds = new Set(
          myRegistrationsResponse.status === "fulfilled"
            ? (myRegistrationsResponse.value.data?.events || []).map((item) =>
                String(item?._id || item?.id || item?.eventId || "").trim()
              )
            : []
        );
        setIsRegistered(
    ...
    try {
      const response = await api({
        ...
        data: {
          ...
          participantProfile: leaderProfile,
      ...
      const headCount =
        Number(response.data?.registration?.headCount) ||
