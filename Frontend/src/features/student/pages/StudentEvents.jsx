...
export default function StudentEvents() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      const [publicEventsResponse, myRegistrationsResponse] = await Promise.allSettled([
        api({ ...SummaryApi.get_public_events }),
        api({ ...SummaryApi.get_my_registered_events }),
      ]);

      if (publicEventsResponse.status !== "fulfilled") {
        throw publicEventsResponse.reason;
      }

      const publicEvents = publicEventsResponse.value.data?.events || [];
      const registeredIds = new Set(
        myRegistrationsResponse.status === "fulfilled"
          ? (myRegistrationsResponse.value.data?.events || []).map((event) =>
              String(event?._id || event?.id || event?.eventId || "").trim()
            )
          : []
      );
