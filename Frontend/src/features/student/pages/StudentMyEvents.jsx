...
export default function StudentMyEvents() {
  ...
  const fetchEvents = async () => {
    ...
    setError(null);
    try {
      const [publicResponse, registeredResponse] = await Promise.all([
        api({ ...SummaryApi.get_public_events }),
        api({ ...SummaryApi.get_my_registered_events }),
      ]);

      const registeredRaw = registeredResponse.data?.events || [];
      const registeredIds = new Set(
        registeredRaw.map((event) => String(event?._id || event?.id || event?.eventId || "").trim())
      );
      ...
      const registeredMapped = registeredRaw
        .map((event) => mapApiEventToCard(event, { registeredIds }))
