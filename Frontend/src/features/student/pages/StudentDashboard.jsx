...
export default function StudentDashboard() {
  ...
  const fetchDashboardEvents = async () => {
    ...
    try {
      ...
      const myRaw = myResponse.data?.events || [];
      const registeredIds = new Set(
        myRaw.map((item) => String(item?._id || item?.id || item?.eventId || "").trim())
      );

      const allMapped = (publicResponse.data?.events || [])
  ...
  const handleRegister = async (eventId) => {
    ...
    try {
      ...
      if (registeredEvent && !myEvents.some((event) => event.id === eventId)) {
        setMyEvents((prev) => [{ ...registeredEvent, isRegistered: true }, ...prev]);
      }
    } catch (err) {
