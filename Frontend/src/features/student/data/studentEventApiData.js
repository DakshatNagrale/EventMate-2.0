...
const fallbackRequirements = (event) => {
  ...
  const participationMode = String(event?.registration?.participationMode || "INDIVIDUAL").toUpperCase();
  const maxTeamMembers = Number(event?.registration?.maxTeamMembers || 4);
  const requirements = [];
  ...
  if (participationMode !== "INDIVIDUAL" && Number.isFinite(maxTeamMembers) && maxTeamMembers > 1) {
    requirements.push({
      title: "Team Limit",
      description: `Maximum ${maxTeamMembers} members per team (including leader).`,
    });
  }
...
export const mapApiEventToDetails = (event) => {
  ...
  return {
    ...
    participationMode,
    maxTeamMembers:
      Number.isFinite(maxTeamMembers) && maxTeamMembers >= 2 ? Math.floor(maxTeamMembers) : 4,
